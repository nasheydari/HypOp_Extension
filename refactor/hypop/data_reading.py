...
def read_headers_constraints(path, task, logger):
    if task == "uf":
        constraints, header = read_uf(path)
    elif task == "stanford" or task == "random_reg" or task == "bipartite" :
        constraints, header = read_stanford(path)
    elif task == "hypergraph":
        ...
        logger.warning('Data mode does not exist. Only support uf, stanford, and hypergraph')
