...
from hypop.loss import loss_maxcut_numpy_boost, loss_sat_numpy_boost, loss_maxind_numpy_boost, loss_maxind_QUBO, loss_task_numpy, loss_task_numpy_vec, loss_mincut_numpy_boost, loss_watermark
...
def centralized_solver(constraints, header, params: Params, file_name):
    ...
    if params.data == 'bipartite':
        path = params.folder_path + file_name[:-14]+'.txt'
    ...
    if params.data != 'task':
        ...
        edges = [[abs(x) - 1 for x in edge[:-1]] for edge in constraints]
    ...
    if params.random_init=='none' and not load:
        ...
        ###### for HyperGNN (hypergraph problems) ######
            ...
            name_g=params.G_folder + params.mode+'_'+file_name[:-4]+".npy"
            np.save(name_g,G)
            G = torch.from_numpy(G).float()

    elif load:
        name_g=params.G_folder + params.mode+'_'+file_name[:-4]+".npy" #### will have to add to the config file #####
        G=np.load(name_g)
        G = torch.from_numpy(G).float()
    ...
    for i in range(params.K): ## if we want to solve the problem for K number of times ##
        ...
        if params.mode == 'task_vec': ## resource allocation problem ##
            ...
            res, prob, train_time, map_time = centralized_train_vec_task(G, params, f, constraints, n, info, weights, file_name, C_dic, lenc, leninfo)
            ...
            res, prob, train_time, map_time = centralized_train_bipartite(G, params, f, constraints_hyper, n,
                                                                          n_hyper, info_hyper, weights, file_name)
            train_times.append(train_time)
            map_times.append(map_time)
